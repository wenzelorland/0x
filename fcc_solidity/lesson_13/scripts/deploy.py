...
def deploy_token_farm_and_dapp_token(front_end_update=False):
    ...
    # send the dapp token supply to the token_farm
    tx = dapp_token.transfer(token_farm.address, dapp_token.totalSupply() - KEPT_BALANCE, {"from":account})
    tx.wait(1)

    # dapp_token, weth_token, fau_token (faucet token) / dai
...
def update_front_end():
    ...
    # This is so that the front-end has the corrent addresses that are stored within the brownie-config.
